...
export function SubMenu({ animal }) {
    ...
    return (
        <div
            className={`h-[50vh] w-fit fixed top-4 z-[999] bg-blue-600/30 backdrop-blur-xl border border-white/15
                rounded-2xl shadow-xl shadow-black/30 overflow-y-auto overscroll-contain transition-all duration-200 ease-out
                ${fade
...
export function Header({ open, setOpen, approachedFrom }) {
    ...
    useEffect(() => {
        if (contactHeightRef.current) {
            const height = contactHeightRef.current.getBoundingClientRect().height;
            setContactHeight(height + 10); // 10px gap below the target
        }
    }, []);

    // adjusting the login display box height
    useEffect(() => {
        if (loginHeightRef.current) {
            const height = loginHeightRef.current.getBoundingClientRect().height;
            setLoginHeight(height + 12); // 12px gap below the target
        }
    }, []);
        ...
        <>
            ...
            >
                ...
                <div className="upper_header grid grid-cols-[0.2fr_1.8fr] grid-rows-2 sm:flex sm:flex-row justify-center sm:justify-evenly items-center gap-3 py-3 w-[100vw] text-[#0A3D62]">
                    ...
                    {/* Right Upper Header */}
                    <div className="r-upper-header col-span-2 flex flex-row justify-evenly items-center gap-4 w-[100vw] sm:gap-4 sm:mt-0 sm:w-fit h-fit">
...
// 19.15572555683343, 72.83593154618605
function MapEmbed() {
    // const destination = "19.15572555683343, 72.83593154618605"; 
